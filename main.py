    ...
    try:
        ...
        if cloudtype == 'moodle':
            ...
            if loged:
                ...
                bot.editMessageText(message,'😩La página no me responde😩')
        ...
        if '/adduser' in msgText:
            ...
            if isadmin:
                try:
                    ...
                    bot.sendMessage(update.message.chat.id,'😎🖕Usted no es mi CREADOR😎🖕')
            else:
                bot.sendMessage(update.message.chat.id,'😎🖕Usted no es mi CREADOR😎🖕')
            return
        if '/banuser' in msgText:
            ...
            if isadmin:
                try:
                    user = str(msgText).split(' ')[1]
                    if user == username:
                        bot.sendMessage(update.message.chat.id,'😒Usted es IMBÉSIL😒')
                        return
                    jdb.remove(user)
                    jdb.save()
                    msg = '👋🏻Adiós @'+user+' Baneado✅'
                    bot.sendMessage(update.message.chat.id,msg)
                except:
                    bot.sendMessage(update.message.chat.id,'❌Error en el comando /banuser username❌')
            else:
                bot.sendMessage(update.message.chat.id,'😳No Tienes Permisos😳')
            return
        if '/getdb' in msgText:
            isadmin = jdb.is_admin(username)
            if isadmin:
        ...
        if '/zips' in msgText:
            ...
            if getUser:
                try:
                   ...
                   jdb.save()
                   msg = '😃👌LISTO. Ahora los zips son de '+ sizeof_fmt(size*1024*1024)+' cada partes✅'
                   bot.sendMessage(update.message.chat.id,msg)
                except:
        ...
        if '/start' in msgText:
            ...
            bot.editMessageText(message,start_msg)
             ...
             if loged:
                ...
                bot.editMessageText(message,'❌Error por Causas como 😔👇\n1-Revise su Cuenta👤\n2-Problemas en el servidor📡: '+client.path)
            ...
            #    return
            bot.editMessageText(message,'Ese comando no existe 😂')
    except Exception as ex:
           print(str(ex))
...
def main():
    ...
    #set in debug
    bot_token = '6024782713:AAGr2gX3b1VUP_4-Ye4Zq8kthsebrXQtqlA'
